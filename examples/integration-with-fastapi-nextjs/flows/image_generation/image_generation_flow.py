...
async def generate_image(enhanced_prompt: EnhancedImagePrompt) -> str:
    ...
    try:
        ...
        if enhanced_prompt.model_type == "dalle":
            ...
            # For DALL-E, we only use the main prompt and size
            ...
            # For Stable Diffusion, we use all parameters including style_preset
            if enhanced_prompt.style_preset not in STABLE_DIFFUSION_CONFIG["styles"]:
                logger.warning(f"Invalid style preset: {enhanced_prompt.style_preset}, defaulting to 'photographic'")
                style_preset = "photographic"
            else:
                style_preset = enhanced_prompt.style_preset
