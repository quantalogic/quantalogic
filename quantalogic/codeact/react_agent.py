...
class ReActAgent:
    ...
    async def _finalize_step(self, task: str, history: List[Dict], step_data: Dict, 
        ...
        if is_complete:
            try:
                # Parse the existing result XML and append FinalAnswer only if it doesn't exist
                root = etree.fromstring(step_data["result"])
                if root.find("FinalAnswer") is None:  # Check if FinalAnswer already exists
                ...
                # Fallback to string concatenation if parsing fails, only if no FinalAnswer exists
