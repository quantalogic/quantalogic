    ...
    def __init__(self, conversation_manager: ConversationManager, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        try:
            super().__init__(
                ...
                description="Retrieve the content of a message in the conversation history by its id.",
                arguments=[
                    ToolArgument(name="nanoid", arg_type="string", description="The nanoid of the message to retrieve", required=True)
                ],
                return_type="string"
            )
        ...
        try:
            ...
            for msg in self.conversation_manager.messages:
                ...
                if msg.role == 'assistant' and len(msg.content) > 100:  # Only consider substantive assistant messages
                    ...
                    if any(id_part in msg.content[:50].lower() for id_part in [nanoid.lower(), nanoid.lower().replace('-', '')]):
