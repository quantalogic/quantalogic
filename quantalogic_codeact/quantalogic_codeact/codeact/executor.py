...
class Executor(BaseExecutor):
            ...
            async def wrapped_tool_with_confirm(**kwargs):
                ...
                if not requires_confirmation and hasattr(tool, "_func"):
                    if hasattr(tool._func, "requires_confirmation"):
                        ...
                        logger.debug(
                            f"Propagated requires_confirmation={requires_confirmation} from function to tool {tool.name}"
                        )
                ...
                if requires_confirmation:
                    ...
                    # 2. Try tool's confirmation_message attribute
                        ...
                        if callable(tool.confirmation_message):
                            ...
                            logger.debug(
                                f"Got confirmation message via callable tool.confirmation_message for {tool.name}"
                            )
                        else:
                            confirmation_message = tool.confirmation_message
                            logger.debug(
                                f"Got confirmation message via static tool.confirmation_message for {tool.name}"
                            )
                    ...
                    # 3. Try the underlying function
                        ...
                        if callable(tool._func.confirmation_message):
                            ...
                            logger.debug(
                                f"Got confirmation message via callable _func.confirmation_message for {tool.name}"
                            )
                        else:
                            confirmation_message = tool._func.confirmation_message
                            logger.debug(
                                f"Got confirmation message via static _func.confirmation_message for {tool.name}"
                            )
    ...
    def register_tool(self, tool: Tool) -> None:
        ...
        setattr(
            self.tool_namespace[toolbox_name], tool.name, self._wrap_tool(tool) if self.verbose else tool.async_execute
        )
    ...
    async def execute_action(self, code: str, context_vars: Dict, step: int, timeout: int = 300) -> ExecutionResult:
        ...
        if not validate_code(code):
            ...
            return ExecutionResult(execution_status="error", error="Code lacks async main()", execution_time=0.0)
        ...
        try:
            ...
            if result.error:
                ...
                return ExecutionResult(
                    execution_status="error", error=err_str, execution_time=result.execution_time or 0.0
                )
